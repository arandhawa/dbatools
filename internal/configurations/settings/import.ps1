...
Set-DbatoolsConfig -Name 'Import.enableDebugMode' -Value $false -Initialize -Validation bool -Handler {
    try {
        ...
        if ($args[0]) {
            $null = New-ItemProperty "HKCU:\SOFTWARE\Microsoft\WindowsPowerShell\dbatools\System" -Name enableDebugMode -PropertyType DWORD -Value 1 -Force -ErrorAction Stop
        } else {
            $null = New-ItemProperty "HKCU:\SOFTWARE\Microsoft\WindowsPowerShell\dbatools\System" -Name enableDebugMode -PropertyType DWORD -Value 0 -Force -ErrorAction Stop
        }
        # Scope Boundary exception: $cfg is defined in Set-DbatoolsConfig
        Register-DbatoolsConfig -Config $cfg
    } catch {
        Write-Message -Level Warning -Message "Failed to apply configuration 'Import.enableDebugMode'" -ErrorRecord $_ -Target 'Import.enableDebugMode'
    }
